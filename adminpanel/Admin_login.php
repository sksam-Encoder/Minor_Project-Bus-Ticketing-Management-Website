...
if (isset($_POST['login_btn'])) {
  ...
  if (strlen($email) < 5  || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
    ...
    echo $email;
    echo $password;
    $res = $user2->DetectAdmin($email, $password);

    if ($res == 1) {
      $_SESSION['email'] = $email;

      $name = $user2->con->db->query("SELECT name FROM admin_data  WHERE email='$email' And password='$password' ");
      $na = mysqli_fetch_array($name);
      $naam = $na['name'];
